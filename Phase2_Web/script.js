...
// Network logic states
let isRemoteLinked = false;

// 1. Navigation Flow
function enableRemoteControl() {
    isRemoteLinked = true;
    const rcNode = document.getElementById('remote-control-config');
    if (rcNode) rcNode.style.display = 'block';
    startWebHub();
}

function sendRemoteData(payload) {
    if (!isRemoteLinked) return;
    const ip = document.getElementById('pc-ip-address').value;
    if (!ip) return;
    ...
    }).catch(e => {}); // Silent catch logic loop limits
}
...
function onResults(results) {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        if (!isPaused) {
            ...
            // Phase 4 Blast coordinates over HTTP to PC server
            sendRemoteData({action: 'move', x: 1 - indexTip.x, y: indexTip.y});
        ...
        if ((indexUp && middleUp && !ringUp && !pinkyUp && distRight > 0.08 && distLeft > 0.08) || 
            ...
            if (indexTip.y < 0.35) {
                ...
                sendRemoteData({action: 'scroll', dy: -40});
            }
        ...
        if (currentMode === 'mouse') {
            if ((distLeft < 0.05 && !middleUp && !ringUp) || (distLeft < 0.06 && distRight > 0.08)) {
                if (!isLeftClicking) {
                    ...
                    // Network blast Phase 4 click
                    sendRemoteData({action: 'click'});
                }
            ...
            if ((distRight < 0.05 && !indexUp && !ringUp) || (distRight < 0.06 && distLeft > 0.08)) {
                if (!isRightClicking) {
                    ...
                    sendRemoteData({action: 'right_click'});
                }
